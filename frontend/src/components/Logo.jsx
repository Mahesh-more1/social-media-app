...
function Logo({ setSelectedTab, className = "h-16 w-16" }) {
  const uniqueId = React.useMemo(
    () => Math.random().toString(36).slice(2, 9),
    []
  );
  const gradientId = `cubeGradient-${uniqueId}`;
  const shadowId = `shadowGradient-${uniqueId}`;
  ...
  return (
    <>
      <svg
        ...
        onClick={() => setSelectedTab && setSelectedTab("Home")}
      >
        <defs>
        ...
        {/* Shadow */}
        <ellipse cx="70" cy="105" rx="20" ry="5" fill={`url(#${shadowId})`} />

        {/* 3D Cube Faces */}
        <path d="M30 40 L70 20 L70 60 L30 80 Z" fill={`url(#${gradientId})`} />
        <path
